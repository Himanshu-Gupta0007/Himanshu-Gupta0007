# 👋 Hi, I'm Himanshu Gupta

### 🚀 MERN Stack Developer | Full Stack Developer | React.js | Node.js | AI Integration

I'm a **MERN Stack Developer with 8 months of professional experience**, currently working as a **Full Stack Developer at DB Services**. I build responsive, scalable and real-world web applications using modern JavaScript technologies.

---

## 💼 Professional Experience

### Full Stack Developer — DB Services

**New Delhi | 8 Months Experience**

* Developed full-stack applications using **React.js, Node.js, Express.js & MongoDB**
* Built responsive UIs with **React.js, Tailwind CSS & reusable components**
* Developed REST APIs, authentication and backend services
* Worked with **MongoDB & Mongoose** for database management
* Integrated third-party and **AI APIs**
* Used **Git/GitHub, Postman, Vercel & Render**
* Debugged production issues and optimized application performance
* Worked on real-world business applications and feature development

---

## 🛠️ Tech Stack

**Frontend:** React.js • Next.js • JavaScript • TypeScript • Tailwind CSS • Redux Toolkit

**Backend:** Node.js • Express.js • REST APIs • JWT • Mongoose

**Database:** MongoDB • MongoDB Atlas

**Tools:** Git • GitHub • Postman • VS Code • Vercel • Render

**AI:** Gemini API • AI API Integration • Generative AI Features

---

## 🚀 Featured Projects

### 🎬 ShowStack — Movie Ticket Booking

Full-stack movie booking application with movie listings, shows, authentication and booking functionality.

**Tech:** React.js • Tailwind CSS • Node.js • Express.js • MongoDB

🔗 **Live Demo:** https://show-stack.vercel.app/
🔗 **GitHub:** https://github.com/Himanshu-Gupta0007/ShowStack

---

### 🤖 TechSphere AI

AI-powered SaaS web application with modern UI and AI-based features including thumbnail generation.

**Tech:** React.js • Tailwind CSS • AI APIs

🔗 **GitHub:** https://github.com/Himanshu-Gupta0007

---

### 🏢 Corporate Mitra Portal

Business management web application with responsive frontend, backend APIs and MongoDB database.

**Tech:** React.js • Tailwind CSS • Node.js • Express.js • MongoDB

🔗 **Live Demo:** https://corporatemitraportal.com/
🔗 **GitHub:** https://github.com/Himanshu-Gupta0007

---

## 📊 Core Skills

* Full Stack Web Development
* MERN Stack Development
* REST API Development
* Authentication & Authorization
* Database Design
* API Integration
* Responsive UI Development
* AI Integration
* Git & GitHub
* Deployment & Debugging

---

## 🎓 Education

**Bachelor of Computer Applications (BCA)**

---

## 📫 Connect With Me

💼 **LinkedIn:**
https://www.linkedin.com/in/himanshu-gupta-546a5b323/

💻 **GitHub:**
https://github.com/Himanshu-Gupta0007

📧 **Open to:** Full Stack Developer • MERN Stack Developer • React.js Developer opportunities

---

### ⭐ Thanks for visiting my profile!

**Let's build something amazing together! 🚀**
